...
| `pyodide-bytecode.zip` | bytecode for the numpy and scipy modules the page imports, unpacked into site-packages after the wheels are installed. Compiled at build time *inside Pyodide*, because a `.pyc` is only valid for the interpreter that wrote it and this machine's Python is not the browser's — see `scripts/precompile.mjs` |
